...
/** NOVO: mapeia o texto do chip para tom semântico */
function pillTone(text: string) {
  const v = text.toLowerCase();
  if (v.includes("não informado") || v === "—")
    return { fill: "#F1F5F9", stroke: "#E2E8F0", text: "#475569" }; // cinza
  if (v === "não")
    return { fill: "#FEF2F2", stroke: "#FECACA", text: "#B91C1C" }; // vermelho
  if (v === "sim")
    return { fill: "#ECFDF5", stroke: "#BBF7D0", text: "#047857" }; // verde
  if (["às vezes", "parcialmente", "em parte", "raramente", "talvez"].includes(v))
    return { fill: "#FFFBEB", stroke: "#FDE68A", text: "#B45309" }; // âmbar (parcial)
  // padrão azul
  return { fill: "#F6F9FF", stroke: "#E0E7FF", text: BRAND_BLUE };
}

/** ATUALIZADO: chip com cores semânticas e um pouco mais alto */
...
/** ATUALIZADO: layout com altura mínima e mais “respiro” entre blocos */
function renderDetailedAsCards(
  ...
  const MIN_CARD_H = 300;       // mantém cartões alinhados
  const TITLE_TO_ID = 12;       // espaço título → identificação
  const SECTION_TITLE_GAP = 10; // título da seção → chips
  const BETWEEN_SECTIONS = 14;  // espaço entre blocos
  const BETWEEN_ROWS = 12;      // antes do comentário
  ...
  answers.forEach((a, idx) => {
    ...
    // Comentário para cálculo de altura
    ...
    // Identificação (se houver consentimento)
    ...
    // Ajuste real se o conteúdo ficou maior
    ...
    // próxima linha quando fechar a grade
